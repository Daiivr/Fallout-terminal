...
function setTopTabActive(view) {
  ...
  elements.tabDrops?.classList.toggle("active", view === "events");
...
function syncTopTabForCurrentView() {
  if (state.fo76Events.open || state.view === "events") {
    setTopTabActive("events");
...
function hideDropsPage() {
  ...
  document.body.classList.remove("is-drops", "is-drops-modal-open");
  if (elements.dropsPage) {
    elements.dropsPage.classList.remove("is-entering");
    elements.dropsPage.hidden = true;
    elements.dropsPage.setAttribute("aria-hidden", "true");
  }
  if (elements.dropsAdminFloatingBtn) {
    elements.dropsAdminFloatingBtn.classList.remove("is-active");
    elements.dropsAdminFloatingBtn.setAttribute("aria-expanded", "false");
  }
}

function canUseDropsAdminModal(profile = state.files.me) {
  const me = normalizeFilesProfile(profile);
  return Boolean(me.loggedIn && hasFilesAuthorizedAccess(me) && me.isAdmin && canUseFilesBotAdmin(me, { requireDesktop: true }));
}

function closeDropsAdminModal({ force = false } = {}) {
  if (!force && !document.body.classList.contains("is-drops-modal-open")) {
    return;
  }
  hideDropsPage();
  renderFilesBotAdminPanel();
...
function mountDropsAdminModalNode() {
  if (!elements.dropsPage || elements.dropsPage.parentElement === document.body) {
    return;
  }
  document.body.appendChild(elements.dropsPage);
}

function mountFo76EventsPageNode() {
  if (!elements.fo76EventsOverlay || !elements.classifiedPage) {
    return;
  }
  if (elements.fo76EventsOverlay.nextElementSibling === elements.classifiedPage) {
    return;
  }
  elements.classifiedPage.before(elements.fo76EventsOverlay);
}

function hideFo76EventsPage() {
  if (!document.body.classList.contains("is-events")) {
    return;
  }
  document.body.classList.remove("is-events");
...
function openDropsAdminModal() {
  const me = normalizeFilesProfile(state.files.me);
  if (!canUseDropsAdminModal(me)) {
    renderFilesBotAdminPanel();
    return;
  }
  ...
  closeFilesAdminModal();
  mountDropsAdminModalNode();
  document.body.classList.add("is-drops-modal-open");

  if (elements.dropsPage) {
    elements.dropsPage.hidden = false;
    elements.dropsPage.setAttribute("aria-hidden", "false");
    elements.dropsPage.classList.remove("is-entering");
    elements.dropsPage.scrollTop = 0;
    elements.dropsPage.scrollLeft = 0;
  }
...
function closeClassifiedModalsForNavigation() {
  ...
  closeFo76EventInfoModal();
  closeFo76MinervaListModal();
  closeFo76EventsModal();
...
function showFo76EventsPage({ updateHash = true } = {}) {
  closeIntelBotInviteModal();
  hideSiloDossier({ updateHash: false });
  closeClassifiedPageForNavigation();
  hideFilesPage();
  ...
  closeFo76MinervaListModal();
  closeFo76EventInfoModal();
  mountFo76EventsPageNode();
  document.body.classList.add("is-events");
  state.view = "events";
  state.fo76Events.open = false;
  syncVisitCounterEyeMode();
  elements.mainTitle.textContent = t("fo76_events_title");
  renderFo76EventsModal();
  syncTopTabForCurrentView();
  renderFilesBotAdminPanel();
  if (!state.fo76Events.data) {
    void fetchFo76Events();
  }
  if (updateHash) {
    setHashView("events");
  }
...
function applyViewFromHash() {
  ...
  if (hashView === "events") {
    ...
    showFo76EventsPage({ updateHash: false });
    return;
  }
...
function renderFilesBotAdminPanel() {
  ...
  if (elements.dropsAdminFloatingBtn) {
    const dropsModalOpen = document.body.classList.contains("is-drops-modal-open");
    elements.dropsAdminFloatingBtn.hidden = !showIndexButton;
    elements.dropsAdminFloatingBtn.classList.toggle("is-active", dropsModalOpen);
    elements.dropsAdminFloatingBtn.setAttribute("aria-expanded", dropsModalOpen ? "true" : "false");
    elements.dropsAdminFloatingBtn.setAttribute("aria-label", t("drops_admin_fab_title"));
  }
  if (elements.filesBotAdminModalBtn) {
...
async function confirmDropsDeleteModal() {
  ...
  try {
    ...
    closeDropsDeleteModal({ force: true });
    setDropsUploadFeedback(t("drops_success_deleted"), "success");
    await refreshDrops();
  } catch (error) {
    closeDropsDeleteModal({ force: true });
    setDropsUploadFeedback(String(error?.message || t("drops_error_delete")), "error");
    renderDropsPage();
  }
}
...
function formatDropsUploadFileName(name) {
  const value = String(name || "").trim();
  if (!value) {
    return t("drops_no_file_selected");
  }
...
function buildDropVirusBadgeState(entry) {
  ...
  const detectionsLabel = t("drops_virus_detections", { count: String(detected) });

  if (status === "clean") {
    return {
      className: "is-safe",
      title: t("drops_virus_safe"),
      body: detectionsLabel
    };
  }
  if (status === "flagged") {
    return {
      className: "is-flagged",
      title: t("drops_virus_flagged"),
      body: detectionsLabel
    };
  }
  if (status === "pending" || status === "queued") {
    return {
      ...
      title: t("drops_virus_scanning"),
      body: t("drops_virus_pending")
    };
  }
  if (status === "error") {
    return {
      className: "is-error",
      title: t("drops_virus_error"),
      body: t("drops_virus_unavailable")
    };
  }
  if (status === "skipped") {
    return {
      className: "is-muted",
      title: t("drops_virus_skipped"),
      body: t("drops_virus_not_scanned")
    };
  }
  return {
    className: "is-muted",
    title: t("drops_virus_unchecked"),
    body: t("drops_virus_not_scanned")
  };
}
...
function renderDropsAdminTabVisibility() {
  ...
  elements.tabDrops.hidden = false;
  elements.tabDrops.setAttribute("aria-hidden", "false");
}
...
function renderDropsList() {
  ...
  elements.dropsEmptyState.textContent = t("drops_empty_state");
  elements.dropsEmptyState.dataset.title = t("drops_empty_state_title");
  elements.dropsEmptyState.dataset.body = t("drops_empty_state_body");

  if (state.drops.loading) {
    elements.dropsListMeta.textContent = t("drops_list_refreshing");
    ...
    const vtLabel = state.drops.virusTotalConfigured ? t("drops_vt_enabled") : t("drops_vt_disabled");
    elements.dropsListMeta.textContent = t("drops_list_meta", {
      limit: uploadLimitLabel,
      hours: String(state.drops.retentionMaxHours || "--"),
      vt: vtLabel
    });
  }
  ...
  for (const entry of entries) {
    ...
    const metaRows = [
      [
        { label: t("drops_meta_type"), value: String(entry.mimeType || "").trim() || "--" },
        { label: t("drops_meta_size"), value: formatFileSize(entry.size) }
      ],
      [
        { label: t("drops_meta_uploaded"), value: formatFileDateTime(entry.uploadedAt) },
        { label: t("drops_meta_expires"), value: entry.expiresAt ? formatFileDateTime(entry.expiresAt) : t("drops_download_limit_only") }
      ],
      [
        {
          label: t("drops_meta_downloads"),
          value: entry.maxDownloads > 0
            ? `${entry.downloadCount} / ${entry.maxDownloads}`
            : t("drops_downloads_used", { count: String(entry.downloadCount) })
        },
        entry.uploader ? { label: t("drops_meta_uploader"), value: entry.uploader } : null
      ].filter(Boolean)
    ];
    ...
    description.textContent = entry.description || t("drops_no_public_description");
    ...
    copyButton.textContent = t("drops_copy_link");
    ...
    openLink.textContent = t("drops_open_link");
    actions.appendChild(openLink);

    const deleteButton = document.createElement("button");
    deleteButton.type = "button";
    deleteButton.className = "files-btn drops-item-btn is-delete";
    deleteButton.textContent = t("drops_delete_link");
...
function renderDropsPage() {
  ...
  if (showGate) {
    const message = loadingIdentity
      ? t("drops_gate_checking")
      : !me.loggedIn
        ? t("drops_gate_login")
        : t("drops_gate_admin_only");
  ...
  if (elements.dropsUploadBtn) {
    ...
    elements.dropsUploadBtn.textContent = state.drops.uploadBusy
      ? t("drops_upload_busy")
      : hasActiveShare
        ? t("drops_upload_locked")
        : t("drops_upload_submit");
  }
  ...
  if (elements.dropsExpiryHint) {
    elements.dropsExpiryHint.textContent = expiryMode === "datetime"
      ? t("drops_expiration_hint_datetime")
      : t("drops_expiration_hint_hours");
  }
  if (elements.dropsExpiresAtInput) {
    const now = new Date();
...
function startDropsCountAutoPoll() {
  ...
  dropsCountPollTimer = setInterval(async () => {
    const dropsVisible = state.view === "drops" || document.body.classList.contains("is-drops-modal-open");
    if (!dropsVisible) {
      stopDropsCountAutoPoll();
      return;
    }
...
function startDropsVtAutoPoll() {
  ...
  dropsVtPollTimer = setInterval(async () => {
    const dropsVisible = state.view === "drops" || document.body.classList.contains("is-drops-modal-open");
    if (!dropsVisible || !dropsHasPendingVtScans()) {
      stopDropsVtAutoPoll();
      return;
    }
    await refreshDrops({ silent: true });
...
async function refreshDrops({ silent = false } = {}) {
  ...
  try {
    ...
    state.drops.list = [];
    state.drops.error = String(error?.message || t("drops_error_load"));
  } finally {
    if (!silent) {
      state.drops.loading = false;
    }
...
async function handleDropsUploadSubmit() {
  ...
  if (Array.isArray(state.drops.list) && state.drops.list.length > 0) {
    setDropsUploadFeedback(t("drops_error_delete_current"), "error");
    renderDropsPage();
    return;
  }
  ...
  if (!file) {
    setDropsUploadFeedback(t("drops_error_missing_file"), "error");
    renderDropsPage();
    return;
  }
  if (state.drops.uploadLimitBytes > 0 && Number(file.size) > state.drops.uploadLimitBytes) {
    setDropsUploadFeedback(t("drops_error_file_too_large", { limit: formatFileSize(state.drops.uploadLimitBytes) }), "error");
    renderDropsPage();
    return;
  }
  if (expiryMode === "datetime" && expiresAtRaw) {
    if (!expiresAtIso) {
      setDropsUploadFeedback(t("drops_error_invalid_expiration"), "error");
      renderDropsPage();
      return;
    }
    if (expiresAtMs <= Date.now() + 30 * 1000) {
      setDropsUploadFeedback(t("drops_error_past_expiration"), "error");
      renderDropsPage();
      return;
    }
    if (maxRetentionMs > 0 && expiresAtMs - Date.now() > maxRetentionMs) {
      setDropsUploadFeedback(t("drops_error_retention_limit", { hours: String(state.drops.retentionMaxHours) }), "error");
      renderDropsPage();
      return;
    }
  }
  if (maxDownloads <= 0 && expiresInHours <= 0 && !expiresAtIso) {
    setDropsUploadFeedback(t("drops_error_missing_limit"), "error");
    renderDropsPage();
    return;
  }
  ...
  try {
    ...
    syncDropsUploadFileName();
    setDropsUploadFeedback(t("drops_success_created"), "success");
    await refreshDrops();
  } catch (error) {
    resetDropsUploadProgress();
    setDropsUploadFeedback(String(error?.message || t("drops_error_create")), "error");
  } finally {
...
async function handleDropsListClick(event) {
  ...
  if (action === "copy") {
    try {
      ...
      setDropsUploadFeedback(t("drops_success_copied"), "success");
    } catch {
      setDropsUploadFeedback(t("drops_error_copy"), "error");
    }
    renderDropsPage();
    return;
  }
...
async function refreshFilesIdentity({ loadFiles = true } = {}) {
  ...
  if (document.body.classList.contains("is-drops-modal-open") && !hasActiveAdminAccess) {
    closeDropsAdminModal({ force: true });
  }
...
async function refreshFilesIdentityBadgeOnly() {
  ...
  renderDropsAdminTabVisibility();
  syncClassifiedAccessState();
  if (document.body.classList.contains("is-drops-modal-open") && (!state.files.me.isAdmin || state.files.localAccessExpired)) {
    closeDropsAdminModal({ force: true });
  }
  if (state.view === "drops" && document.body.classList.contains("is-drops")) {
...
function applyLanguage(lang, persist = true) {
  ...
  if (elements.dropsAdminFloatingBtnLabel) {
    elements.dropsAdminFloatingBtnLabel.textContent = t("drops_admin_fab_label");
  }
  if (elements.dropsAdminFloatingBtnHint) {
    elements.dropsAdminFloatingBtnHint.textContent = t("drops_admin_fab_hint");
  }
  if (elements.dropsAdminFloatingBtn) {
    elements.dropsAdminFloatingBtn.removeAttribute("title");
    elements.dropsAdminFloatingBtn.setAttribute("aria-label", t("drops_admin_fab_title"));
  }
  if (elements.dropsModalCloseBtn) {
    elements.dropsModalCloseBtn.setAttribute("aria-label", t("drops_admin_modal_close"));
    elements.dropsModalCloseBtn.textContent = t("files_admin_modal_close");
  }
  if (elements.dropsModalBadge) {
    elements.dropsModalBadge.textContent = t("drops_modal_badge");
  }
  if (elements.dropsModalTitle) {
    elements.dropsModalTitle.textContent = t("drops_modal_title");
  }
  if (elements.dropsModalSubtitle) {
    elements.dropsModalSubtitle.textContent = t("drops_modal_subtitle");
  }
  if (elements.dropsUploadKicker) {
    elements.dropsUploadKicker.textContent = t("drops_upload_kicker");
  }
  if (elements.dropsUploadTitle) {
    elements.dropsUploadTitle.textContent = t("drops_upload_title");
  }
  if (elements.dropsLangLabel) {
    elements.dropsLangLabel.textContent = t("drops_lang_label");
  }
  if (elements.dropsFileLabel) {
    elements.dropsFileLabel.textContent = t("drops_file_label");
  }
  if (elements.dropsUploadPickerBtn) {
    elements.dropsUploadPickerBtn.textContent = t("drops_choose_file");
  }
  if (elements.dropsDisplayNameLabel) {
    elements.dropsDisplayNameLabel.textContent = t("drops_display_name_label");
  }
  if (elements.dropsDisplayNameInput) {
    elements.dropsDisplayNameInput.placeholder = t("drops_display_name_placeholder");
  }
  if (elements.dropsMaxDownloadsLabel) {
    elements.dropsMaxDownloadsLabel.textContent = t("drops_max_downloads_label");
  }
  if (elements.dropsMaxDownloadsInput) {
    elements.dropsMaxDownloadsInput.placeholder = t("drops_max_downloads_placeholder");
  }
  if (elements.dropsExpiryLabel) {
    elements.dropsExpiryLabel.textContent = t("drops_expiration_label");
  }
  if (elements.dropsExpiryModeHoursBtn) {
    elements.dropsExpiryModeHoursBtn.textContent = t("drops_expiration_mode_hours");
  }
  if (elements.dropsExpiryModeDateBtn) {
    elements.dropsExpiryModeDateBtn.textContent = t("drops_expiration_mode_datetime");
  }
  if (elements.dropsExpiresHoursInput) {
    elements.dropsExpiresHoursInput.placeholder = t("drops_expiration_hours_placeholder");
  }
  if (elements.dropsDescriptionLabel) {
    elements.dropsDescriptionLabel.textContent = t("drops_description_label");
  }
  if (elements.dropsDescriptionInput) {
    elements.dropsDescriptionInput.placeholder = t("drops_description_placeholder");
  }
  if (elements.dropsListKicker) {
    elements.dropsListKicker.textContent = t("drops_list_kicker");
  }
  if (elements.dropsListTitle) {
    elements.dropsListTitle.textContent = t("drops_list_title");
  }
  ...
  } else if (document.body.classList.contains("is-events")) {
    elements.mainTitle.textContent = t("fo76_events_title");
  } else if (document.body.classList.contains("is-drops") && !document.body.classList.contains("is-drops-modal-open")) {
    elements.mainTitle.textContent = t("drops_main_title");
...
function wireEvents() {
  ...
  const fo76EventsModalRoot = elements.fo76EventsCore || null;
  const fo76RoadMapModalRoot = elements.fo76RoadMapCore || null;
  const fo76MinervaListModalRoot = elements.fo76MinervaListCore || null;
  const fo76EventInfoModalRoot = elements.fo76EventInfoCore || null;
  ...
  const dropsAdminModalRoot = elements.dropsAdminView || elements.dropsGatePanel || null;
  const shouldBlockBackgroundForActiveOverlay = (target) => {
    ...
    if (elements.fo76MinervaListOverlay?.classList.contains("is-active")) {
      if (!(target instanceof Node) || !(fo76MinervaListModalRoot instanceof Node)) {
        return true;
      }
      return !fo76MinervaListModalRoot.contains(target);
    }
    if (elements.fo76EventInfoOverlay?.classList.contains("is-active")) {
      if (!(target instanceof Node) || !(fo76EventInfoModalRoot instanceof Node)) {
        return true;
      }
      return !fo76EventInfoModalRoot.contains(target);
    }
    if (elements.fo76RoadMapOverlay?.classList.contains("is-active")) {
      if (!(target instanceof Node) || !(fo76RoadMapModalRoot instanceof Node)) {
        return true;
      }
      return !fo76RoadMapModalRoot.contains(target);
    }
    if (elements.fo76EventsOverlay?.classList.contains("is-active") && state.fo76Events.open) {
      if (!(target instanceof Node) || !(fo76EventsModalRoot instanceof Node)) {
        return true;
      }
      return !fo76EventsModalRoot.contains(target);
    }
    if (document.body.classList.contains("is-drops-modal-open")) {
      if (!(target instanceof Node) || !(dropsAdminModalRoot instanceof Node)) {
        return true;
      }
      return !dropsAdminModalRoot.contains(target) && target !== elements.dropsModalCloseBtn;
    }
  ...
  elements.fo76EventsBtn?.addEventListener("click", () => {
    openFo76EventsModal();
  });
  elements.fo76EventsRefreshBtn?.addEventListener("click", () => {
    void fetchFo76Events({ force: true });
  });
  elements.fo76RoadMapBtn?.addEventListener("click", () => {
    openFo76RoadMapModal();
  });
  elements.fo76EventsCloseBtn?.addEventListener("click", () => {
    closeFo76EventsModal();
  });
  elements.fo76EventsCloseIconBtn?.addEventListener("click", () => {
    closeFo76EventsModal();
  });
  elements.fo76EventsContent?.addEventListener("error", handleFo76EventIconError, true);
  elements.fo76EventsContent?.addEventListener("mouseover", handleFo76EventsTooltipEnter);
  elements.fo76EventsContent?.addEventListener("mouseout", handleFo76EventsTooltipLeave);
  elements.fo76EventsContent?.addEventListener("focusin", handleFo76EventsTooltipEnter);
  elements.fo76EventsContent?.addEventListener("focusout", handleFo76EventsTooltipLeave);
  elements.fo76EventsContent?.addEventListener("click", handleFo76EventsContentClick);
  elements.fo76EventsContent?.addEventListener("scroll", positionFo76EventsTooltip, { passive: true });
  window.addEventListener("resize", positionFo76EventsTooltip);
  window.addEventListener("scroll", positionFo76EventsTooltip, { passive: true });
  elements.fo76EventsOverlay?.addEventListener("click", (event) => {
    if (state.fo76Events.open && event.target === elements.fo76EventsOverlay) {
      closeFo76EventsModal();
    }
  });
  elements.fo76RoadMapCloseBtn?.addEventListener("click", () => {
    closeFo76RoadMapModal();
  });
  elements.fo76RoadMapLensInput?.addEventListener("input", handleFo76RoadMapLensZoomInput);
  elements.fo76RoadMapFrame?.addEventListener("pointerenter", handleFo76RoadMapLensPointerMove);
  elements.fo76RoadMapFrame?.addEventListener("pointermove", handleFo76RoadMapLensPointerMove);
  elements.fo76RoadMapFrame?.addEventListener("pointerleave", hideFo76RoadMapLens);
  elements.fo76RoadMapFrame?.addEventListener("scroll", hideFo76RoadMapLens, { passive: true });
  elements.fo76RoadMapFrame?.addEventListener("wheel", handleFo76RoadMapLensWheel, { passive: false });
  elements.fo76RoadMapCloseIconBtn?.addEventListener("click", () => {
    closeFo76RoadMapModal();
  });
  elements.fo76RoadMapOverlay?.addEventListener("click", (event) => {
    if (event.target === elements.fo76RoadMapOverlay) {
      closeFo76RoadMapModal();
    }
  });
  elements.fo76MinervaListContent?.addEventListener("click", handleFo76MinervaListContentClick);
  elements.fo76MinervaListCloseBtn?.addEventListener("click", () => {
    closeFo76MinervaListModal();
  });
  elements.fo76MinervaListCloseIconBtn?.addEventListener("click", () => {
    closeFo76MinervaListModal();
  });
  elements.fo76MinervaListOverlay?.addEventListener("click", (event) => {
    if (event.target === elements.fo76MinervaListOverlay) {
      closeFo76MinervaListModal();
    }
  });
  elements.fo76EventInfoCloseBtn?.addEventListener("click", () => {
    closeFo76EventInfoModal();
  });
  elements.fo76EventInfoCloseIconBtn?.addEventListener("click", () => {
    closeFo76EventInfoModal();
  });
  elements.fo76EventInfoOverlay?.addEventListener("click", (event) => {
    if (event.target === elements.fo76EventInfoOverlay) {
      closeFo76EventInfoModal();
    }
  });
  ...
  elements.dropsAdminFloatingBtn?.addEventListener("click", () => {
    if (document.body.classList.contains("is-drops-modal-open")) {
      closeDropsAdminModal();
      return;
    }
    openDropsAdminModal();
  });
  elements.dropsModalCloseBtn?.addEventListener("click", () => {
    closeDropsAdminModal();
  });
