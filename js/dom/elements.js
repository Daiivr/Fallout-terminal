globalThis.FALLOUT_CODEX_ELEMENTS = {
  bootOverlay: document.getElementById("bootOverlay"),
  bootTitle: document.getElementById("bootTitle"),
  bootSubtitle: document.getElementById("bootSubtitle"),
  bootLog: document.getElementById("bootLog"),
  bootBar: document.getElementById("bootBar"),
  bootPercent: document.getElementById("bootPercent"),
  bootHint: document.getElementById("bootHint"),
  bootLine1: document.getElementById("bootLine1"),
  bootLine2: document.getElementById("bootLine2"),
  bootLine3: document.getElementById("bootLine3"),
  bootReady: document.getElementById("bootReady"),
  hackOverlay: document.getElementById("hackOverlay"),
  hackTitle: document.getElementById("hackTitle"),
  hackSubtitle: document.getElementById("hackSubtitle"),
  hackAttemptsLabel: document.getElementById("hackAttemptsLabel"),
  hackAttempts: document.getElementById("hackAttempts"),
  hackMemory: document.getElementById("hackMemory"),
  hackLog: document.getElementById("hackLog"),
  hackAbortBtn: document.getElementById("hackAbortBtn"),
  hackRetryBtn: document.getElementById("hackRetryBtn"),
  hackOpenClassifiedBtn: document.getElementById("hackOpenClassifiedBtn"),
  filesDeleteOverlay: document.getElementById("filesDeleteOverlay"),
  filesDeleteTitle: document.getElementById("filesDeleteTitle"),
  filesDeleteMessage: document.getElementById("filesDeleteMessage"),
  filesDeleteCancelBtn: document.getElementById("filesDeleteCancelBtn"),
  filesDeleteConfirmBtn: document.getElementById("filesDeleteConfirmBtn"),
  dropsDeleteOverlay: document.getElementById("dropsDeleteOverlay"),
  dropsDeleteBadge: document.getElementById("dropsDeleteBadge"),
  dropsDeleteTitle: document.getElementById("dropsDeleteTitle"),
  dropsDeleteMessage: document.getElementById("dropsDeleteMessage"),
  dropsDeleteCancelBtn: document.getElementById("dropsDeleteCancelBtn"),
  dropsDeleteConfirmBtn: document.getElementById("dropsDeleteConfirmBtn"),
  filesCautionOverlay: document.getElementById("filesCautionOverlay"),
  filesCautionTitle: document.getElementById("filesCautionTitle"),
  filesCautionMessage: document.getElementById("filesCautionMessage"),
  filesCautionRejectBtn: document.getElementById("filesCautionRejectBtn"),
  filesCautionConfirmBtn: document.getElementById("filesCautionConfirmBtn"),
  filesShareOverlay: document.getElementById("filesShareOverlay"),
  filesShareBadge: document.getElementById("filesShareBadge"),
  filesShareTitle: document.getElementById("filesShareTitle"),
  filesShareMessage: document.getElementById("filesShareMessage"),
  filesSharePrivateBtn: document.getElementById("filesSharePrivateBtn"),
  filesSharePrivateTitle: document.getElementById("filesSharePrivateTitle"),
  filesSharePrivateBody: document.getElementById("filesSharePrivateBody"),
  filesSharePublicBtn: document.getElementById("filesSharePublicBtn"),
  filesSharePublicTitle: document.getElementById("filesSharePublicTitle"),
  filesSharePublicBody: document.getElementById("filesSharePublicBody"),
  filesSharePublicForm: document.getElementById("filesSharePublicForm"),
  filesShareCodeLabel: document.getElementById("filesShareCodeLabel"),
  filesShareCodeInput: document.getElementById("filesShareCodeInput"),
  filesSharePublicCreateBtn: document.getElementById("filesSharePublicCreateBtn"),
  filesSharePublicHint: document.getElementById("filesSharePublicHint"),
  filesShareFeedback: document.getElementById("filesShareFeedback"),
  filesShareCancelBtn: document.getElementById("filesShareCancelBtn"),
  filesDetailOverlay: document.getElementById("filesDetailOverlay"),
  filesDetailModalCloseBtn: document.getElementById("filesDetailModalCloseBtn"),
  filesDetailModalPanel: document.getElementById("filesDetailModalPanel"),
  filesDetailModalBody: document.getElementById("filesDetailModalBody"),
  filesBotAdminLeaveOverlay: document.getElementById("filesBotAdminLeaveOverlay"),
  filesBotAdminLeaveBadge: document.getElementById("filesBotAdminLeaveBadge"),
  filesBotAdminLeaveTitle: document.getElementById("filesBotAdminLeaveTitle"),
  filesBotAdminLeaveMessage: document.getElementById("filesBotAdminLeaveMessage"),
  filesBotAdminLeaveCancelBtn: document.getElementById("filesBotAdminLeaveCancelBtn"),
  filesBotAdminLeaveConfirmBtn: document.getElementById("filesBotAdminLeaveConfirmBtn"),
  filesBotAdminDiagnosticsOverlay: document.getElementById("filesBotAdminDiagnosticsOverlay"),
  filesBotAdminDiagnosticsModalBadge: document.getElementById("filesBotAdminDiagnosticsModalBadge"),
  filesBotAdminDiagnosticsModalTitle: document.getElementById("filesBotAdminDiagnosticsModalTitle"),
  filesBotAdminDiagnosticsModalHint: document.getElementById("filesBotAdminDiagnosticsModalHint"),
  filesBotAdminDiagnosticsModalMeta: document.getElementById("filesBotAdminDiagnosticsModalMeta"),
  filesBotAdminDiagnosticsModalBody: document.getElementById("filesBotAdminDiagnosticsModalBody"),
  filesBotAdminDiagnosticsModalCloseBtn: document.getElementById("filesBotAdminDiagnosticsModalCloseBtn"),
  filesBotAdminServerOverlay: document.getElementById("filesBotAdminServerOverlay"),
  filesBotAdminServerModalBadge: document.getElementById("filesBotAdminServerModalBadge"),
  filesBotAdminServerModalTitle: document.getElementById("filesBotAdminServerModalTitle"),
  filesBotAdminServerModalHint: document.getElementById("filesBotAdminServerModalHint"),
  filesBotAdminServerModalMeta: document.getElementById("filesBotAdminServerModalMeta"),
  filesBotAdminServerModalBody: document.getElementById("filesBotAdminServerModalBody"),
  filesBotAdminServerModalCloseBtn: document.getElementById("filesBotAdminServerModalCloseBtn"),
  filesDisclaimerOverlay: document.getElementById("filesDisclaimerOverlay"),
  filesDisclaimerTitle: document.getElementById("filesDisclaimerTitle"),
  filesDisclaimerBody1: document.getElementById("filesDisclaimerBody1"),
  filesDisclaimerBody2: document.getElementById("filesDisclaimerBody2"),
  filesDisclaimerCloseBtn: document.getElementById("filesDisclaimerCloseBtn"),
  filesGroupRenameOverlay: document.getElementById("filesGroupRenameOverlay"),
  filesGroupRenameTitle: document.getElementById("filesGroupRenameTitle"),
  filesGroupRenameMessage: document.getElementById("filesGroupRenameMessage"),
  filesGroupRenameForm: document.getElementById("filesGroupRenameForm"),
  filesGroupRenameLabel: document.getElementById("filesGroupRenameLabel"),
  filesGroupRenameInput: document.getElementById("filesGroupRenameInput"),
  filesGroupRenameFeedback: document.getElementById("filesGroupRenameFeedback"),
  filesGroupRenameCancelBtn: document.getElementById("filesGroupRenameCancelBtn"),
  filesGroupRenameConfirmBtn: document.getElementById("filesGroupRenameConfirmBtn"),
  filesUploadOverlay: document.getElementById("filesUploadOverlay"),
  filesUploadModalCloseBtn: document.getElementById("filesUploadModalCloseBtn"),
  filesEditOverlay: document.getElementById("filesEditOverlay"),
  filesEditModalCloseBtn: document.getElementById("filesEditModalCloseBtn"),
  filesFunctionsOverlay: document.getElementById("filesFunctionsOverlay"),
  filesFunctionsModalCloseBtn: document.getElementById("filesFunctionsModalCloseBtn"),
  filesFunctionsPanel: document.getElementById("filesFunctionsPanel"),
  filesFunctionsModalTitle: document.getElementById("filesFunctionsModalTitle"),
  filesFunctionsModalFileName: document.getElementById("filesFunctionsModalFileName"),
  filesFunctionsModalCount: document.getElementById("filesFunctionsModalCount"),
  filesFunctionsModalBody: document.getElementById("filesFunctionsModalBody"),
  filesAdminRequestsOverlay: document.getElementById("filesAdminRequestsOverlay"),
  filesAdminRequestsModalCloseBtn: document.getElementById("filesAdminRequestsModalCloseBtn"),
  filesPublicSharesOverlay: document.getElementById("filesPublicSharesOverlay"),
  filesPublicSharesModalCloseBtn: document.getElementById("filesPublicSharesModalCloseBtn"),
  filesBotAdminOverlay: document.getElementById("filesBotAdminOverlay"),
  filesBotAdminModalCloseBtn: document.getElementById("filesBotAdminModalCloseBtn"),
  syncOverlay: document.getElementById("syncOverlay"),
  syncTitle: document.getElementById("syncTitle"),
  classifiedLoadOverlay: document.getElementById("classifiedLoadOverlay"),
  classifiedLoadTitle: document.getElementById("classifiedLoadTitle"),
  commandTerminalOverlay: document.getElementById("commandTerminalOverlay"),
  commandTerminalCore: document.getElementById("commandTerminalCore"),
  commandTerminalCloseBtn: document.getElementById("commandTerminalCloseBtn"),
  commandTerminalOutput: document.getElementById("commandTerminalOutput"),
  commandTerminalForm: document.getElementById("commandTerminalForm"),
  commandTerminalInput: document.getElementById("commandTerminalInput"),
  commandTerminalBadge: document.getElementById("commandTerminalBadge"),
  commandTerminalTitle: document.getElementById("commandTerminalTitle"),
  commandTerminalHint: document.getElementById("commandTerminalHint"),
  commandTerminalInputLabel: document.getElementById("commandTerminalInputLabel"),
  commandTerminalStatusReady: document.getElementById("commandTerminalStatusReady"),
  commandTerminalStatusEmpty: document.getElementById("commandTerminalStatusEmpty"),
  commandTerminalStatusExit: document.getElementById("commandTerminalStatusExit"),
  commandTerminalPromptUser: document.getElementById("commandTerminalPromptUser"),
  commandTerminalTypedText: document.getElementById("commandTerminalTypedText"),
  intelBotInviteOverlay: document.getElementById("intelBotInviteOverlay"),
  intelBotInviteCore: document.getElementById("intelBotInviteCore"),
  intelBotInviteBadge: document.getElementById("intelBotInviteBadge"),
  intelBotInviteTitle: document.getElementById("intelBotInviteTitle"),
  intelBotInviteBody: document.getElementById("intelBotInviteBody"),
  intelBotInviteCancelBtn: document.getElementById("intelBotInviteCancelBtn"),
  intelBotInviteConfirmBtn: document.getElementById("intelBotInviteConfirmBtn"),
  intelEmailOverlay: document.getElementById("intelEmailOverlay"),
  intelEmailForm: document.getElementById("intelEmailForm"),
  intelEmailBadge: document.getElementById("intelEmailBadge"),
  intelEmailCloseIconBtn: document.getElementById("intelEmailCloseIconBtn"),
  intelEmailTitle: document.getElementById("intelEmailTitle"),
  intelEmailBody: document.getElementById("intelEmailBody"),
  intelEmailFeedLabel: document.getElementById("intelEmailFeedLabel"),
  intelEmailFeedValue: document.getElementById("intelEmailFeedValue"),
  intelEmailAccountPill: document.getElementById("intelEmailAccountPill"),
  intelEmailAccountAvatar: document.getElementById("intelEmailAccountAvatar"),
  intelEmailAccountLabel: document.getElementById("intelEmailAccountLabel"),
  intelEmailAccountName: document.getElementById("intelEmailAccountName"),
  intelEmailLoginPanel: document.getElementById("intelEmailLoginPanel"),
  intelEmailLoginTitle: document.getElementById("intelEmailLoginTitle"),
  intelEmailLoginBody: document.getElementById("intelEmailLoginBody"),
  intelEmailLoginBtn: document.getElementById("intelEmailLoginBtn"),
  intelEmailSubscribedPanel: document.getElementById("intelEmailSubscribedPanel"),
  intelEmailSubscribedTitle: document.getElementById("intelEmailSubscribedTitle"),
  intelEmailSubscribedBody: document.getElementById("intelEmailSubscribedBody"),
  intelEmailSubscribedEmail: document.getElementById("intelEmailSubscribedEmail"),
  intelEmailUnsubscribeBtn: document.getElementById("intelEmailUnsubscribeBtn"),
  intelEmailCooldownPanel: document.getElementById("intelEmailCooldownPanel"),
  intelEmailCooldownTitle: document.getElementById("intelEmailCooldownTitle"),
  intelEmailCooldownBody: document.getElementById("intelEmailCooldownBody"),
  intelEmailCooldownValue: document.getElementById("intelEmailCooldownValue"),
  intelEmailInputLabel: document.getElementById("intelEmailInputLabel"),
  intelEmailInputShell: document.getElementById("intelEmailInputShell"),
  intelEmailInput: document.getElementById("intelEmailInput"),
  intelEmailHint: document.getElementById("intelEmailHint"),
  intelEmailFeedback: document.getElementById("intelEmailFeedback"),
  intelEmailAdminTools: document.getElementById("intelEmailAdminTools"),
  intelEmailAdminToolsLabel: document.getElementById("intelEmailAdminToolsLabel"),
  intelEmailTestConfirmationBtn: document.getElementById("intelEmailTestConfirmationBtn"),
  intelEmailTestIntelBtn: document.getElementById("intelEmailTestIntelBtn"),
  intelEmailAdminSubscribersBtn: document.getElementById("intelEmailAdminSubscribersBtn"),
  intelEmailAdminSubscribersPanel: document.getElementById("intelEmailAdminSubscribersPanel"),
  intelEmailAdminSubscribersList: document.getElementById("intelEmailAdminSubscribersList"),
  intelEmailCancelBtn: document.getElementById("intelEmailCancelBtn"),
  intelEmailSubmitBtn: document.getElementById("intelEmailSubmitBtn"),
  classifiedPlayersOverlay: document.getElementById("classifiedPlayersOverlay"),
  classifiedPlayersCore: document.getElementById("classifiedPlayersCore"),
  classifiedPlayersBadge: document.getElementById("classifiedPlayersBadge"),
  classifiedPlayersTitle: document.getElementById("classifiedPlayersTitle"),
  classifiedPlayersBody: document.getElementById("classifiedPlayersBody"),
  classifiedPlayersMeta: document.getElementById("classifiedPlayersMeta"),
  classifiedPlayersStatus: document.getElementById("classifiedPlayersStatus"),
  classifiedPlayersStats: document.getElementById("classifiedPlayersStats"),
  classifiedPlayersNowValue: document.getElementById("classifiedPlayersNowValue"),
  classifiedPlayersNowLabel: document.getElementById("classifiedPlayersNowLabel"),
  classifiedPlayersPeak24hValue: document.getElementById("classifiedPlayersPeak24hValue"),
  classifiedPlayersPeak24hLabel: document.getElementById("classifiedPlayersPeak24hLabel"),
  classifiedPlayersPeakAllTimeValue: document.getElementById("classifiedPlayersPeakAllTimeValue"),
  classifiedPlayersPeakAllTimeLabel: document.getElementById("classifiedPlayersPeakAllTimeLabel"),
  classifiedPlayersChartPanel: document.getElementById("classifiedPlayersChartPanel"),
  classifiedPlayersChartTitle: document.getElementById("classifiedPlayersChartTitle"),
  classifiedPlayersRangeControls: document.getElementById("classifiedPlayersRangeControls"),
  classifiedPlayersRangeButtons: Array.from(document.querySelectorAll(".classified-player-range-btn")),
  classifiedPlayersChartFrame: document.getElementById("classifiedPlayersChartFrame"),
  classifiedPlayersChartSvg: document.getElementById("classifiedPlayersChartSvg"),
  classifiedPlayersChartInteractive: document.getElementById("classifiedPlayersChartInteractive"),
  classifiedPlayersChartCrosshair: document.getElementById("classifiedPlayersChartCrosshair"),
  classifiedPlayersChartMarker: document.getElementById("classifiedPlayersChartMarker"),
  classifiedPlayersChartTooltip: document.getElementById("classifiedPlayersChartTooltip"),
  classifiedPlayersChartTooltipDate: document.getElementById("classifiedPlayersChartTooltipDate"),
  classifiedPlayersChartTooltipValue: document.getElementById("classifiedPlayersChartTooltipValue"),
  classifiedPlayersChartEmpty: document.getElementById("classifiedPlayersChartEmpty"),
  classifiedPlayersNote: document.getElementById("classifiedPlayersNote"),
  classifiedPlayersSourceLink: document.getElementById("classifiedPlayersSourceLink"),
  classifiedPlayersRefreshBtn: document.getElementById("classifiedPlayersRefreshBtn"),
  classifiedPlayersCloseBtn: document.getElementById("classifiedPlayersCloseBtn"),
  classifiedIntelOverlay: document.getElementById("classifiedIntelOverlay"),
  classifiedIntelCore: document.getElementById("classifiedIntelCore"),
  classifiedIntelBadge: document.getElementById("classifiedIntelBadge"),
  classifiedIntelCloseIconBtn: document.getElementById("classifiedIntelCloseIconBtn"),
  classifiedIntelTitle: document.getElementById("classifiedIntelTitle"),
  classifiedIntelBody: document.getElementById("classifiedIntelBody"),
  classifiedIntelMeta: document.getElementById("classifiedIntelMeta"),
  classifiedIntelStatus: document.getElementById("classifiedIntelStatus"),
  classifiedIntelContent: document.getElementById("classifiedIntelContent"),
  classifiedIntelRefreshBtn: document.getElementById("classifiedIntelRefreshBtn"),
  classifiedIntelCloseBtn: document.getElementById("classifiedIntelCloseBtn"),
  microText: document.getElementById("microText"),
  mainTitle: document.getElementById("mainTitle"),
  tabStatus: document.getElementById("tabStatus"),
  tabStatusText: document.getElementById("tabStatusText"),
  tabStatusDecisionBadge: document.getElementById("tabStatusDecisionBadge"),
  tabIntel: document.getElementById("tabIntel"),
  tabDrops: document.getElementById("tabDrops"),
  tabData: document.getElementById("tabData"),
  discordBotInviteBtn: document.getElementById("discordBotInviteBtn"),
  discordBotInviteLabel: document.getElementById("discordBotInviteLabel"),
  discordBotInviteHint: document.getElementById("discordBotInviteHint"),
  langLabel: document.getElementById("langLabel"),
  langDropdown: document.getElementById("langDropdown"),
  langToggleBtn: document.getElementById("langToggleBtn"),
  langMenu: document.getElementById("langMenu"),
  langCurrent: document.getElementById("langCurrent"),
  langOptions: Array.from(document.querySelectorAll(".lang-option")),
  langSelect: document.getElementById("langSelect"),
  labelUtc: document.getElementById("labelUtc"),
  labelLastSync: document.getElementById("labelLastSync"),
  labelDataLink: document.getElementById("labelDataLink"),
  statusStrip: document.getElementById("statusStrip"),
  utcTime: document.getElementById("utcTime"),
  lastRefresh: document.getElementById("lastRefresh"),
  dataSignal: document.getElementById("dataSignal"),
  refreshBtn: document.getElementById("refreshBtn"),
  intelGrid: document.getElementById("intelGrid"),
  filesPage: document.getElementById("filesPage"),
  dropsPage: document.getElementById("dropsPage"),
  dropsModalCloseBtn: document.getElementById("dropsModalCloseBtn"),
  dropsGatePanel: document.getElementById("dropsGatePanel"),
  dropsGateBadge: document.getElementById("dropsGateBadge"),
  dropsGateTitle: document.getElementById("dropsGateTitle"),
  dropsGateMessage: document.getElementById("dropsGateMessage"),
  dropsAdminView: document.getElementById("dropsAdminView"),
  dropsModalBadge: document.getElementById("dropsModalBadge"),
  dropsModalTitle: document.getElementById("dropsModalTitle"),
  dropsModalSubtitle: document.getElementById("dropsModalSubtitle"),
  dropsUploadKicker: document.getElementById("dropsUploadKicker"),
  dropsUploadTitle: document.getElementById("dropsUploadTitle"),
  dropsUploadLimitsHint: document.getElementById("dropsUploadLimitsHint"),
  dropsUploadForm: document.getElementById("dropsUploadForm"),
  dropsUploadInput: document.getElementById("dropsUploadInput"),
  dropsFileLabel: document.getElementById("dropsFileLabel"),
  dropsUploadPickerBtn: document.getElementById("dropsUploadPickerBtn"),
  dropsUploadFileName: document.getElementById("dropsUploadFileName"),
  dropsDisplayNameLabel: document.getElementById("dropsDisplayNameLabel"),
  dropsDisplayNameInput: document.getElementById("dropsDisplayNameInput"),
  dropsMaxDownloadsLabel: document.getElementById("dropsMaxDownloadsLabel"),
  dropsMaxDownloadsInput: document.getElementById("dropsMaxDownloadsInput"),
  dropsExpiryLabel: document.getElementById("dropsExpiryLabel"),
  dropsExpiryModeHoursBtn: document.getElementById("dropsExpiryModeHoursBtn"),
  dropsExpiryModeDateBtn: document.getElementById("dropsExpiryModeDateBtn"),
  dropsExpiresHoursWrap: document.getElementById("dropsExpiresHoursWrap"),
  dropsExpiresHoursInput: document.getElementById("dropsExpiresHoursInput"),
  dropsExpiresAtWrap: document.getElementById("dropsExpiresAtWrap"),
  dropsExpiresAtInput: document.getElementById("dropsExpiresAtInput"),
  dropsExpiryHint: document.getElementById("dropsExpiryHint"),
  dropsDescriptionLabel: document.getElementById("dropsDescriptionLabel"),
  dropsDescriptionInput: document.getElementById("dropsDescriptionInput"),
  dropsLangLabel: document.getElementById("dropsLangLabel"),
  dropsLangDropdown: document.getElementById("dropsLangDropdown"),
  dropsLangToggleBtn: document.getElementById("dropsLangToggleBtn"),
  dropsLangMenu: document.getElementById("dropsLangMenu"),
  dropsLangCurrent: document.getElementById("dropsLangCurrent"),
  dropsLangOptions: Array.from(document.querySelectorAll(".drops-head-lang-option")),
  dropsLangSelect: document.getElementById("dropsLangSelect"),
  dropsUploadFeedback: document.getElementById("dropsUploadFeedback"),
  dropsUploadBtn: document.getElementById("dropsUploadBtn"),
  dropsListKicker: document.getElementById("dropsListKicker"),
  dropsListTitle: document.getElementById("dropsListTitle"),
  dropsRefreshBtn: document.getElementById("dropsRefreshBtn"),
  dropsListMeta: document.getElementById("dropsListMeta"),
  dropsUploadTelemetry: document.getElementById("dropsUploadTelemetry"),
  dropsUploadTelemetryBadge: document.getElementById("dropsUploadTelemetryBadge"),
  dropsUploadTelemetryFile: document.getElementById("dropsUploadTelemetryFile"),
  dropsUploadTelemetryPercent: document.getElementById("dropsUploadTelemetryPercent"),
  dropsUploadTelemetryBar: document.getElementById("dropsUploadTelemetryBar"),
  dropsUploadTelemetryFill: document.getElementById("dropsUploadTelemetryFill"),
  dropsUploadTelemetryBytes: document.getElementById("dropsUploadTelemetryBytes"),
  dropsUploadTelemetryStatus: document.getElementById("dropsUploadTelemetryStatus"),
  dropsEmptyState: document.getElementById("dropsEmptyState"),
  dropsList: document.getElementById("dropsList"),
  filesUnauthorizedPanel: document.getElementById("filesUnauthorizedPanel"),
  filesUnauthorizedBadge: document.getElementById("filesUnauthorizedBadge"),
  filesUnauthorizedTitle: document.getElementById("filesUnauthorizedTitle"),
  filesUnauthorizedSubtitle: document.getElementById("filesUnauthorizedSubtitle"),
  filesUnauthorizedKicker: document.getElementById("filesUnauthorizedKicker"),
  filesUnauthorizedStateLabel: document.getElementById("filesUnauthorizedStateLabel"),
  filesUnauthorizedStateValue: document.getElementById("filesUnauthorizedStateValue"),
  filesUnauthorizedGateLabel: document.getElementById("filesUnauthorizedGateLabel"),
  filesUnauthorizedGateValue: document.getElementById("filesUnauthorizedGateValue"),
  filesUnauthorizedTraceLabel: document.getElementById("filesUnauthorizedTraceLabel"),
  filesUnauthorizedTraceValue: document.getElementById("filesUnauthorizedTraceValue"),
  filesUnauthorizedDirectiveTitle: document.getElementById("filesUnauthorizedDirectiveTitle"),
  filesUnauthorizedDirectiveLine1: document.getElementById("filesUnauthorizedDirectiveLine1"),
  filesUnauthorizedDirectiveLine2: document.getElementById("filesUnauthorizedDirectiveLine2"),
  filesUnauthorizedDirectiveLine3: document.getElementById("filesUnauthorizedDirectiveLine3"),
  filesNotAuthorizedMessage: document.getElementById("filesNotAuthorizedMessage"),
  filesLoginForm: document.getElementById("filesLoginForm"),
  filesLoginReturnTo: document.getElementById("filesLoginReturnTo"),
  filesLoginBtn: document.getElementById("filesLoginBtn"),
  filesLogoutBtn: document.getElementById("filesLogoutBtn"),
  filesAuthorizedView: document.getElementById("filesAuthorizedView"),
  filesRestrictedView: document.getElementById("filesRestrictedView"),
  filesRestrictedBadge: document.getElementById("filesRestrictedBadge"),
  filesRestrictedIncident: document.getElementById("filesRestrictedIncident"),
  filesRestrictedTitle: document.getElementById("filesRestrictedTitle"),
  filesRestrictedSubtitle: document.getElementById("filesRestrictedSubtitle"),
  filesRestrictedPublicShareNote: document.getElementById("filesRestrictedPublicShareNote"),
  filesRestrictedIdentityLabel: document.getElementById("filesRestrictedIdentityLabel"),
  filesRestrictedIdentityValue: document.getElementById("filesRestrictedIdentityValue"),
  filesRestrictedDiscordLabel: document.getElementById("filesRestrictedDiscordLabel"),
  filesRestrictedDiscordValue: document.getElementById("filesRestrictedDiscordValue"),
  filesRestrictedClearanceLabel: document.getElementById("filesRestrictedClearanceLabel"),
  filesRestrictedClearanceValue: document.getElementById("filesRestrictedClearanceValue"),
  filesRestrictedStatusLabel: document.getElementById("filesRestrictedStatusLabel"),
  filesRestrictedStatusValue: document.getElementById("filesRestrictedStatusValue"),
  filesRestrictedTimeLabel: document.getElementById("filesRestrictedTimeLabel"),
  filesRestrictedTimeValue: document.getElementById("filesRestrictedTimeValue"),
  filesRestrictedDirectiveTitle: document.getElementById("filesRestrictedDirectiveTitle"),
  filesRestrictedDirectiveLine1: document.getElementById("filesRestrictedDirectiveLine1"),
  filesRestrictedDirectiveLine2: document.getElementById("filesRestrictedDirectiveLine2"),
  filesRestrictedDirectiveLine3: document.getElementById("filesRestrictedDirectiveLine3"),
  filesRestrictedReasonSection: document.getElementById("filesRestrictedReasonSection"),
  filesRestrictedReasonLabel: document.getElementById("filesRestrictedReasonLabel"),
  filesRestrictedReasonInput: document.getElementById("filesRestrictedReasonInput"),
  filesRestrictedReasonHint: document.getElementById("filesRestrictedReasonHint"),
  filesRestrictedRetryBtn: document.getElementById("filesRestrictedRetryBtn"),
  filesRestrictedLogoutBtn: document.getElementById("filesRestrictedLogoutBtn"),
  filesRestrictedRequestFeedback: document.getElementById("filesRestrictedRequestFeedback"),
  filesDeniedView: document.getElementById("filesDeniedView"),
  filesDeniedBadge: document.getElementById("filesDeniedBadge"),
  filesDeniedTitle: document.getElementById("filesDeniedTitle"),
  filesDeniedSubtitle: document.getElementById("filesDeniedSubtitle"),
  filesDeniedReasonSection: document.getElementById("filesDeniedReasonSection"),
  filesDeniedReasonLabel: document.getElementById("filesDeniedReasonLabel"),
  filesDeniedReasonValue: document.getElementById("filesDeniedReasonValue"),
  filesDeniedStatusLabel: document.getElementById("filesDeniedStatusLabel"),
  filesDeniedStatusValue: document.getElementById("filesDeniedStatusValue"),
  filesDeniedNextWindowLabel: document.getElementById("filesDeniedNextWindowLabel"),
  filesDeniedNextWindowValue: document.getElementById("filesDeniedNextWindowValue"),
  filesDeniedCountdownLabel: document.getElementById("filesDeniedCountdownLabel"),
  filesDeniedCountdownValue: document.getElementById("filesDeniedCountdownValue"),
  filesDeniedDirectiveTitle: document.getElementById("filesDeniedDirectiveTitle"),
  filesDeniedDirectiveLine1: document.getElementById("filesDeniedDirectiveLine1"),
  filesDeniedDirectiveLine2: document.getElementById("filesDeniedDirectiveLine2"),
  filesDeniedDirectiveLine3: document.getElementById("filesDeniedDirectiveLine3"),
  filesDeniedDirectiveLine4: document.getElementById("filesDeniedDirectiveLine4"),
  filesDeniedLogoutBtn: document.getElementById("filesDeniedLogoutBtn"),
  filesDisclaimerGateView: document.getElementById("filesDisclaimerGateView"),
  filesDisclaimerGateBadge: document.getElementById("filesDisclaimerGateBadge"),
  filesDisclaimerGateTitle: document.getElementById("filesDisclaimerGateTitle"),
  filesDisclaimerGateIntro: document.getElementById("filesDisclaimerGateIntro"),
  filesDisclaimerGateBody1: document.getElementById("filesDisclaimerGateBody1"),
  filesDisclaimerGateBody2: document.getElementById("filesDisclaimerGateBody2"),
  filesDisclaimerGateActions: document.getElementById("filesDisclaimerGateActions"),
  filesDisclaimerAgreeBtn: document.getElementById("filesDisclaimerAgreeBtn"),
  filesDisclaimerDeclineBtn: document.getElementById("filesDisclaimerDeclineBtn"),
  filesDisclaimerDeclinedPanel: document.getElementById("filesDisclaimerDeclinedPanel"),
  filesDisclaimerDeclinedTitle: document.getElementById("filesDisclaimerDeclinedTitle"),
  filesDisclaimerDeclinedMessage: document.getElementById("filesDisclaimerDeclinedMessage"),
  filesDisclaimerContactBtn: document.getElementById("filesDisclaimerContactBtn"),
  filesDisclaimerContactView: document.getElementById("filesDisclaimerContactView"),
  filesDisclaimerContactTitle: document.getElementById("filesDisclaimerContactTitle"),
  filesDisclaimerContactHint: document.getElementById("filesDisclaimerContactHint"),
  filesDisclaimerContactLabel: document.getElementById("filesDisclaimerContactLabel"),
  filesDisclaimerContactInput: document.getElementById("filesDisclaimerContactInput"),
  filesDisclaimerContactCancelBtn: document.getElementById("filesDisclaimerContactCancelBtn"),
  filesDisclaimerContactSendBtn: document.getElementById("filesDisclaimerContactSendBtn"),
  filesDisclaimerGateFeedback: document.getElementById("filesDisclaimerGateFeedback"),
  filesDisclaimerAcceptLoader: document.getElementById("filesDisclaimerAcceptLoader"),
  filesDisclaimerAcceptLoaderText: document.getElementById("filesDisclaimerAcceptLoaderText"),
  filesSessionTitle: document.getElementById("filesSessionTitle"),
  filesSessionBadge: document.getElementById("filesSessionBadge"),
  filesSessionIdentity: document.getElementById("filesSessionIdentity"),
  filesSessionUserLabel: document.getElementById("filesSessionUserLabel"),
  filesSessionUser: document.getElementById("filesSessionUser"),
  filesSessionIdLabel: document.getElementById("filesSessionIdLabel"),
  filesSessionId: document.getElementById("filesSessionId"),
  filesSessionClearanceLabel: document.getElementById("filesSessionClearanceLabel"),
  filesSessionClearance: document.getElementById("filesSessionClearance"),
  filesSessionStateLabel: document.getElementById("filesSessionStateLabel"),
  filesSessionState: document.getElementById("filesSessionState"),
  filesSessionTimerRow: document.getElementById("filesSessionTimerRow"),
  filesSessionTimerLabel: document.getElementById("filesSessionTimerLabel"),
  filesSessionTimer: document.getElementById("filesSessionTimer"),
  filesSessionLogoutBtn: document.getElementById("filesSessionLogoutBtn"),
  filesBotAdminFloatingBtn: document.getElementById("filesBotAdminFloatingBtn"),
  filesBotAdminFloatingBtnLabel: document.getElementById("filesBotAdminFloatingBtnLabel"),
  filesBotAdminFloatingBtnHint: document.getElementById("filesBotAdminFloatingBtnHint"),
  dropsAdminFloatingBtn: document.getElementById("dropsAdminFloatingBtn"),
  dropsAdminFloatingBtnLabel: document.getElementById("dropsAdminFloatingBtnLabel"),
  dropsAdminFloatingBtnHint: document.getElementById("dropsAdminFloatingBtnHint"),
  filesAdminToolsPanel: document.getElementById("filesAdminToolsPanel"),
  filesAdminToolsTitle: document.getElementById("filesAdminToolsTitle"),
  filesAdminToolsHint: document.getElementById("filesAdminToolsHint"),
  filesPublicSharesToolsPanel: document.getElementById("filesPublicSharesToolsPanel"),
  filesPublicSharesToolsTitle: document.getElementById("filesPublicSharesToolsTitle"),
  filesPublicSharesToolsHint: document.getElementById("filesPublicSharesToolsHint"),
  filesAdminConsoleModalBtn: document.getElementById("filesAdminConsoleModalBtn"),
  filesAdminConsoleModalBtnText: document.getElementById("filesAdminConsoleModalBtnText"),
  filesAdminConsoleModalBtnHint: document.getElementById("filesAdminConsoleModalBtnHint"),
  filesAccessControlModalBtn: document.getElementById("filesAccessControlModalBtn"),
  filesAccessControlModalBtnText: document.getElementById("filesAccessControlModalBtnText"),
  filesAccessControlModalBtnHint: document.getElementById("filesAccessControlModalBtnHint"),
  filesAccessControlPendingBadge: document.getElementById("filesAccessControlPendingBadge"),
  filesAdminPublicSharesModalBtn: document.getElementById("filesAdminPublicSharesModalBtn"),
  filesAdminPublicSharesModalBtnText: document.getElementById("filesAdminPublicSharesModalBtnText"),
  filesAdminPublicSharesModalBtnHint: document.getElementById("filesAdminPublicSharesModalBtnHint"),
  filesPublicSharesModalBtn: document.getElementById("filesPublicSharesModalBtn"),
  filesPublicSharesModalBtnText: document.getElementById("filesPublicSharesModalBtnText"),
  filesPublicSharesModalBtnHint: document.getElementById("filesPublicSharesModalBtnHint"),
  filesPublicSharesCountBadge: document.getElementById("filesPublicSharesCountBadge"),
  filesBotAdminModalBtn: document.getElementById("filesBotAdminModalBtn"),
  filesBotAdminModalBtnText: document.getElementById("filesBotAdminModalBtnText"),
  filesUploadPanel: document.getElementById("filesUploadPanel"),
  filesUploadTitle: document.getElementById("filesUploadTitle"),
  filesUploadForm: document.getElementById("filesUploadForm"),
  filesUploadAssetsLabel: document.getElementById("filesUploadAssetsLabel"),
  filesUploadAssetsHint: document.getElementById("filesUploadAssetsHint"),
  filesUploadDossierLabel: document.getElementById("filesUploadDossierLabel"),
  filesUploadDossierHint: document.getElementById("filesUploadDossierHint"),
  filesUploadFileLabel: document.getElementById("filesUploadFileLabel"),
  filesUploadInput: document.getElementById("filesUploadInput"),
  filesUploadImageLabel: document.getElementById("filesUploadImageLabel"),
  filesImageInput: document.getElementById("filesImageInput"),
  filesUploadGroupLabel: document.getElementById("filesUploadGroupLabel"),
  filesGroupInput: document.getElementById("filesGroupInput"),
  filesGroupSuggestDropdown: document.getElementById("filesGroupSuggestDropdown"),
  filesGroupSuggestBtn: document.getElementById("filesGroupSuggestBtn"),
  filesGroupSuggestCurrent: document.getElementById("filesGroupSuggestCurrent"),
  filesGroupSuggestMenu: document.getElementById("filesGroupSuggestMenu"),
  filesUploadDescLabel: document.getElementById("filesUploadDescLabel"),
  filesDescriptionInput: document.getElementById("filesDescriptionInput"),
  filesOutdatedInput: document.getElementById("filesOutdatedInput"),
  filesUploadOutdatedLabel: document.getElementById("filesUploadOutdatedLabel"),
  filesUntestedInput: document.getElementById("filesUntestedInput"),
  filesUploadUntestedLabel: document.getElementById("filesUploadUntestedLabel"),
  filesCautionInput: document.getElementById("filesCautionInput"),
  filesUploadCautionLabel: document.getElementById("filesUploadCautionLabel"),
  filesUploadBtn: document.getElementById("filesUploadBtn"),
  filesUploadFeedback: document.getElementById("filesUploadFeedback"),
  filesEditPanel: document.getElementById("filesEditPanel"),
  filesEditTitle: document.getElementById("filesEditTitle"),
  filesEditHint: document.getElementById("filesEditHint"),
  filesEditTargetName: document.getElementById("filesEditTargetName"),
  filesEditModalBody: document.getElementById("filesEditModalBody"),
  filesEditFeedback: document.getElementById("filesEditFeedback"),
  filesAdminRequestsPanel: document.getElementById("filesAdminRequestsPanel"),
  filesAdminRequestsTitle: document.getElementById("filesAdminRequestsTitle"),
  filesAdminRequestsHint: document.getElementById("filesAdminRequestsHint"),
  filesAdminRequestsStateBadge: document.getElementById("filesAdminRequestsStateBadge"),
  filesAdminRequestsConsoleLabel: document.getElementById("filesAdminRequestsConsoleLabel"),
  filesAdminRequestsConsoleHint: document.getElementById("filesAdminRequestsConsoleHint"),
  filesAdminRequestsRegistryLabel: document.getElementById("filesAdminRequestsRegistryLabel"),
  filesAdminRequestsRegistryHint: document.getElementById("filesAdminRequestsRegistryHint"),
  filesAdminRequestsSearchLabel: document.getElementById("filesAdminRequestsSearchLabel"),
  filesAdminRequestsSearchInput: document.getElementById("filesAdminRequestsSearchInput"),
  filesAdminRequestsFilterLabel: document.getElementById("filesAdminRequestsFilterLabel"),
  filesAdminRequestsFilterDropdown: document.getElementById("filesAdminRequestsFilterDropdown"),
  filesAdminRequestsFilterBtn: document.getElementById("filesAdminRequestsFilterBtn"),
  filesAdminRequestsFilterCurrent: document.getElementById("filesAdminRequestsFilterCurrent"),
  filesAdminRequestsFilterMenu: document.getElementById("filesAdminRequestsFilterMenu"),
  filesAdminRequestsFilter: document.getElementById("filesAdminRequestsFilter"),
  filesAdminRequestsFilterPending: document.getElementById("filesAdminRequestsFilterPending"),
  filesAdminRequestsFilterApproved: document.getElementById("filesAdminRequestsFilterApproved"),
  filesAdminRequestsFilterDeclined: document.getElementById("filesAdminRequestsFilterDeclined"),
  filesAdminRequestsFilterAuthorized: document.getElementById("filesAdminRequestsFilterAuthorized"),
  filesAdminRequestsFilterAll: document.getElementById("filesAdminRequestsFilterAll"),
  filesAdminRequestsFilterOptions: Array.from(document.querySelectorAll("#filesAdminRequestsFilterMenu .files-admin-requests-filter-option")),
  filesAdminRequestsRefreshBtn: document.getElementById("filesAdminRequestsRefreshBtn"),
  filesAdminRequestsFeedback: document.getElementById("filesAdminRequestsFeedback"),
  filesAdminRequestsList: document.getElementById("filesAdminRequestsList"),
  filesPublicSharesPanel: document.getElementById("filesPublicSharesPanel"),
  filesPublicSharesBadge: document.getElementById("filesPublicSharesBadge"),
  filesPublicSharesTitle: document.getElementById("filesPublicSharesTitle"),
  filesPublicSharesHint: document.getElementById("filesPublicSharesHint"),
  filesPublicSharesStateBadge: document.getElementById("filesPublicSharesStateBadge"),
  filesPublicSharesConsoleLabel: document.getElementById("filesPublicSharesConsoleLabel"),
  filesPublicSharesConsoleHint: document.getElementById("filesPublicSharesConsoleHint"),
  filesPublicSharesRefreshBtn: document.getElementById("filesPublicSharesRefreshBtn"),
  filesPublicSharesFeedback: document.getElementById("filesPublicSharesFeedback"),
  filesPublicSharesList: document.getElementById("filesPublicSharesList"),
  filesBotAdminPanel: document.getElementById("filesBotAdminPanel"),
  filesBotAdminBadge: document.getElementById("filesBotAdminBadge"),
  filesBotAdminTitle: document.getElementById("filesBotAdminTitle"),
  filesBotAdminHint: document.getElementById("filesBotAdminHint"),
  filesBotAdminMeta: document.getElementById("filesBotAdminMeta"),
  filesBotAdminFeedback: document.getElementById("filesBotAdminFeedback"),
  filesBotAdminRefreshBtn: document.getElementById("filesBotAdminRefreshBtn"),
  filesBotAdminSyncBtn: document.getElementById("filesBotAdminSyncBtn"),
  filesBotAdminDiagnosticsBtn: document.getElementById("filesBotAdminDiagnosticsBtn"),
  filesBotAdminInviteLink: document.getElementById("filesBotAdminInviteLink"),
  filesBotAdminStatusLabel: document.getElementById("filesBotAdminStatusLabel"),
  filesBotAdminStatusValue: document.getElementById("filesBotAdminStatusValue"),
  filesBotAdminServersLabel: document.getElementById("filesBotAdminServersLabel"),
  filesBotAdminServersValue: document.getElementById("filesBotAdminServersValue"),
  filesBotAdminUsersLabel: document.getElementById("filesBotAdminUsersLabel"),
  filesBotAdminUsersValue: document.getElementById("filesBotAdminUsersValue"),
  filesBotAdminChannelsLabel: document.getElementById("filesBotAdminChannelsLabel"),
  filesBotAdminChannelsValue: document.getElementById("filesBotAdminChannelsValue"),
  filesBotAdminOrphansLabel: document.getElementById("filesBotAdminOrphansLabel"),
  filesBotAdminOrphansValue: document.getElementById("filesBotAdminOrphansValue"),
  filesBotAdminOverview: document.getElementById("filesBotAdminOverview"),
  filesBotAdminToolbar: document.getElementById("filesBotAdminToolbar"),
  filesBotAdminSearchLabel: document.getElementById("filesBotAdminSearchLabel"),
  filesBotAdminSearchInput: document.getElementById("filesBotAdminSearchInput"),
  filesBotAdminSortLabel: document.getElementById("filesBotAdminSortLabel"),
  filesBotAdminSortDropdown: document.getElementById("filesBotAdminSortDropdown"),
  filesBotAdminSortBtn: document.getElementById("filesBotAdminSortBtn"),
  filesBotAdminSortCurrent: document.getElementById("filesBotAdminSortCurrent"),
  filesBotAdminSortMenu: document.getElementById("filesBotAdminSortMenu"),
  filesBotAdminSortSelect: document.getElementById("filesBotAdminSortSelect"),
  filesBotAdminSortMembers: document.getElementById("filesBotAdminSortMembers"),
  filesBotAdminSortSubscriptions: document.getElementById("filesBotAdminSortSubscriptions"),
  filesBotAdminSortName: document.getElementById("filesBotAdminSortName"),
  filesBotAdminSortOptions: Array.from(document.querySelectorAll("[data-files-bot-sort]")),
  filesBotAdminFilterWrap: document.getElementById("filesBotAdminFilterWrap"),
  filesBotAdminFilterLabel: document.getElementById("filesBotAdminFilterLabel"),
  filesBotAdminFilterAll: document.getElementById("filesBotAdminFilterAll"),
  filesBotAdminFilterSubscribed: document.getElementById("filesBotAdminFilterSubscribed"),
  filesBotAdminFilterEmpty: document.getElementById("filesBotAdminFilterEmpty"),
  filesBotAdminFilterOptions: Array.from(document.querySelectorAll("[data-files-bot-filter]")),
  filesBotAdminServerList: document.getElementById("filesBotAdminServerList"),
  filesBrowserPanel: document.querySelector("#filesAuthorizedView .files-browser-panel"),
  filesBrowserTitle: document.getElementById("filesBrowserTitle"),
  filesSearchToggleBtn: document.getElementById("filesSearchToggleBtn"),
  filesSearchToggleText: document.getElementById("filesSearchToggleText"),
  filesGroupManagerToggleBtn: document.getElementById("filesGroupManagerToggleBtn"),
  filesGroupManagerToggleText: document.getElementById("filesGroupManagerToggleText"),
  filesDisclaimerBtn: document.getElementById("filesDisclaimerBtn"),
  filesDisclaimerBtnText: document.getElementById("filesDisclaimerBtnText"),
  filesSearchWrap: document.getElementById("filesSearchWrap"),
  filesGroupManagerWrap: document.getElementById("filesGroupManagerWrap"),
  filesSearchLabel: document.getElementById("filesSearchLabel"),
  filesSearchCount: document.getElementById("filesSearchCount"),
  filesSearchInput: document.getElementById("filesSearchInput"),
  filesSearchHint: document.getElementById("filesSearchHint"),
  filesSearchResults: document.getElementById("filesSearchResults"),
  filesEmptyState: document.getElementById("filesEmptyState"),
  filesList: document.getElementById("filesList"),
  siloTitle: document.getElementById("siloTitle"),
  siloEmailBtn: document.getElementById("siloEmailBtn"),
  siloHint: document.getElementById("siloHint"),
  siloExpiry: document.getElementById("siloExpiry"),
  siloCodes: document.getElementById("siloCodes"),
  siloSourcePrefix: document.getElementById("siloSourcePrefix"),
  siloSourceSuffix: document.getElementById("siloSourceSuffix"),
  siloDossierOverlay: document.getElementById("siloDossierOverlay"),
  siloDossierEyebrow: document.getElementById("siloDossierEyebrow"),
  siloDossierTitle: document.getElementById("siloDossierTitle"),
  siloDossierSummary: document.getElementById("siloDossierSummary"),
  siloDossierSourceLink: document.getElementById("siloDossierSourceLink"),
  siloDossierCloseBtn: document.getElementById("siloDossierCloseBtn"),
  siloDossierCodes: document.getElementById("siloDossierCodes"),
  siloDossierResetLabel: document.getElementById("siloDossierResetLabel"),
  siloDossierResetValue: document.getElementById("siloDossierResetValue"),
  siloDossierCountdownLabel: document.getElementById("siloDossierCountdownLabel"),
  siloDossierCountdownValue: document.getElementById("siloDossierCountdownValue"),
  siloDossierStatusLabel: document.getElementById("siloDossierStatusLabel"),
  siloDossierStatusValue: document.getElementById("siloDossierStatusValue"),
  siloDossierSignalLabel: document.getElementById("siloDossierSignalLabel"),
  siloDossierSignalValue: document.getElementById("siloDossierSignalValue"),
  siloDossierBriefing: document.getElementById("siloDossierBriefing"),
  siloDossierBackBtn: document.getElementById("siloDossierBackBtn"),
  minervaTitle: document.getElementById("minervaTitle"),
  minervaEmailBtn: document.getElementById("minervaEmailBtn"),
  minervaSummary: document.getElementById("minervaSummary"),
  minervaLocationLabel: document.getElementById("minervaLocationLabel"),
  minervaListLabel: document.getElementById("minervaListLabel"),
  minervaWindowLabel: document.getElementById("minervaWindowLabel"),
  minervaLocationCardBtn: document.getElementById("minervaLocationCardBtn"),
  minervaLocation: document.getElementById("minervaLocation"),
  minervaList: document.getElementById("minervaList"),
  minervaWindow: document.getElementById("minervaWindow"),
  minervaInventoryTitle: document.getElementById("minervaInventoryTitle"),
  tableItemHeader: document.getElementById("tableItemHeader"),
  tablePriceHeader: document.getElementById("tablePriceHeader"),
  minervaItems: document.getElementById("minervaItems"),
  minervaAwaiting: document.getElementById("minervaAwaiting"),
  minervaSourcePrefix: document.getElementById("minervaSourcePrefix"),
  minervaSourceSuffix: document.getElementById("minervaSourceSuffix"),
  minervaPanel: document.getElementById("minervaPanel"),
  minervaDetailView: document.getElementById("minervaDetailView"),
  minervaDetailBackBtn: document.getElementById("minervaDetailBackBtn"),
  minervaDetailWikiLink: document.getElementById("minervaDetailWikiLink"),
  minervaDetailName: document.getElementById("minervaDetailName"),
  minervaDetailStatus: document.getElementById("minervaDetailStatus"),
  minervaDetailContent: document.getElementById("minervaDetailContent"),
  minervaDetailImage: document.getElementById("minervaDetailImage"),
  minervaDetailWhereLabel: document.getElementById("minervaDetailWhereLabel"),
  minervaDetailWhereList: document.getElementById("minervaDetailWhereList"),
  minervaDetailUnlocksLabel: document.getElementById("minervaDetailUnlocksLabel"),
  minervaDetailUnlocks: document.getElementById("minervaDetailUnlocks"),
  minervaLocationView: document.getElementById("minervaLocationView"),
  minervaLocationBackBtn: document.getElementById("minervaLocationBackBtn"),
  minervaLocationTitle: document.getElementById("minervaLocationTitle"),
  minervaLocationStatus: document.getElementById("minervaLocationStatus"),
  minervaLocationMapLabel: document.getElementById("minervaLocationMapLabel"),
  minervaLocationMapImage: document.getElementById("minervaLocationMapImage"),
  minervaLocationMapPrevBtn: document.getElementById("minervaLocationMapPrevBtn"),
  minervaLocationMapNextBtn: document.getElementById("minervaLocationMapNextBtn"),
  minervaLocationPinsWrap: document.getElementById("minervaLocationPins"),
  minervaLocationMapName: document.getElementById("minervaLocationMapName"),
  minervaLocationArrivesLabel: document.getElementById("minervaLocationArrivesLabel"),
  minervaLocationArrives: document.getElementById("minervaLocationArrives"),
  minervaLocationLeavesLabel: document.getElementById("minervaLocationLeavesLabel"),
  minervaLocationLeaves: document.getElementById("minervaLocationLeaves"),
  minervaLocationCountdownLabel: document.getElementById("minervaLocationCountdownLabel"),
  minervaLocationCountdown: document.getElementById("minervaLocationCountdown"),
  minervaLocationPins: Array.from(document.querySelectorAll(".minerva-loc-pin")),
  classifiedPage: document.getElementById("classifiedPage"),
  classifiedTitle: document.getElementById("classifiedTitle"),
  classifiedWarning: document.getElementById("classifiedWarning"),
  classifiedBackBtn: document.getElementById("classifiedBackBtn"),
  classifiedPlayersBtn: document.getElementById("classifiedPlayersBtn"),
  classifiedDailyOpsBtn: document.getElementById("classifiedDailyOpsBtn"),
  classifiedDailyChallengesBtn: document.getElementById("classifiedDailyChallengesBtn"),
  classifiedWeeklyChallengesBtn: document.getElementById("classifiedWeeklyChallengesBtn"),
  classifiedIntelButtons: Array.from(document.querySelectorAll("[data-classified-intel]")),
  classifiedCard1Title: document.getElementById("classifiedCard1Title"),
  classifiedCard1Body: document.getElementById("classifiedCard1Body"),
  classifiedCard1Action: document.getElementById("classifiedCard1Action"),
  classifiedCard2Title: document.getElementById("classifiedCard2Title"),
  classifiedCard2Body: document.getElementById("classifiedCard2Body"),
  classifiedCard2Action: document.getElementById("classifiedCard2Action"),
  classifiedCard3Title: document.getElementById("classifiedCard3Title"),
  classifiedCard3Body: document.getElementById("classifiedCard3Body"),
  classifiedCard3Action: document.getElementById("classifiedCard3Action"),
  classifiedCard4Title: document.getElementById("classifiedCard4Title"),
  classifiedCard4Body: document.getElementById("classifiedCard4Body"),
  classifiedCard4Action: document.getElementById("classifiedCard4Action"),
  classifiedAxolotlBtn: document.getElementById("classifiedAxolotlBtn"),
  classifiedAxolotlOverlay: document.getElementById("classifiedAxolotlOverlay"),
  classifiedAxolotlCore: document.getElementById("classifiedAxolotlCore"),
  classifiedAxolotlBadge: document.getElementById("classifiedAxolotlBadge"),
  classifiedAxolotlTitle: document.getElementById("classifiedAxolotlTitle"),
  classifiedAxolotlBody: document.getElementById("classifiedAxolotlBody"),
  classifiedAxolotlContent: document.getElementById("classifiedAxolotlContent"),
  classifiedAxolotlSourceLink: document.getElementById("classifiedAxolotlSourceLink"),
  classifiedAxolotlCloseBtn: document.getElementById("classifiedAxolotlCloseBtn"),
  classifiedAxolotlCloseIconBtn: document.getElementById("classifiedAxolotlCloseIconBtn"),
  fo76EventsBtn: document.getElementById("fo76EventsBtn"),
  fo76EventsOverlay: document.getElementById("fo76EventsOverlay"),
  fo76EventsCore: document.getElementById("fo76EventsCore"),
  fo76EventsBadge: document.getElementById("fo76EventsBadge"),
  fo76EventsTitle: document.getElementById("fo76EventsTitle"),
  fo76EventsBody: document.getElementById("fo76EventsBody"),
  fo76EventsMeta: document.getElementById("fo76EventsMeta"),
  fo76EventsStatus: document.getElementById("fo76EventsStatus"),
  fo76EventsContent: document.getElementById("fo76EventsContent"),
  fo76EventsRefreshBtn: document.getElementById("fo76EventsRefreshBtn"),
  fo76RoadMapBtn: document.getElementById("fo76RoadMapBtn"),
  fo76EventsCloseBtn: document.getElementById("fo76EventsCloseBtn"),
  fo76EventsCloseIconBtn: document.getElementById("fo76EventsCloseIconBtn"),
  fo76RoadMapOverlay: document.getElementById("fo76RoadMapOverlay"),
  fo76RoadMapCore: document.getElementById("fo76RoadMapCore"),
  fo76RoadMapBadge: document.getElementById("fo76RoadMapBadge"),
  fo76RoadMapTitle: document.getElementById("fo76RoadMapTitle"),
  fo76RoadMapBody: document.getElementById("fo76RoadMapBody"),
  fo76RoadMapFrame: document.getElementById("fo76RoadMapFrame"),
  fo76RoadMapImage: document.getElementById("fo76RoadMapImage"),
  fo76RoadMapLens: document.getElementById("fo76RoadMapLens"),
  fo76RoadMapLensLabel: document.getElementById("fo76RoadMapLensLabel"),
  fo76RoadMapLensInput: document.getElementById("fo76RoadMapLensInput"),
  fo76RoadMapLensValue: document.getElementById("fo76RoadMapLensValue"),
  fo76RoadMapCloseBtn: document.getElementById("fo76RoadMapCloseBtn"),
  fo76RoadMapCloseIconBtn: document.getElementById("fo76RoadMapCloseIconBtn"),
  fo76MinervaListOverlay: document.getElementById("fo76MinervaListOverlay"),
  fo76MinervaListCore: document.getElementById("fo76MinervaListCore"),
  fo76MinervaListBadge: document.getElementById("fo76MinervaListBadge"),
  fo76MinervaListTitle: document.getElementById("fo76MinervaListTitle"),
  fo76MinervaListBody: document.getElementById("fo76MinervaListBody"),
  fo76MinervaListStatus: document.getElementById("fo76MinervaListStatus"),
  fo76MinervaListContent: document.getElementById("fo76MinervaListContent"),
  fo76MinervaListCloseBtn: document.getElementById("fo76MinervaListCloseBtn"),
  fo76MinervaListCloseIconBtn: document.getElementById("fo76MinervaListCloseIconBtn"),
  fo76EventInfoOverlay: document.getElementById("fo76EventInfoOverlay"),
  fo76EventInfoCore: document.getElementById("fo76EventInfoCore"),
  fo76EventInfoBadge: document.getElementById("fo76EventInfoBadge"),
  fo76EventInfoTitle: document.getElementById("fo76EventInfoTitle"),
  fo76EventInfoBody: document.getElementById("fo76EventInfoBody"),
  fo76EventInfoContent: document.getElementById("fo76EventInfoContent"),
  fo76EventInfoCloseBtn: document.getElementById("fo76EventInfoCloseBtn"),
  fo76EventInfoCloseIconBtn: document.getElementById("fo76EventInfoCloseIconBtn"),
  classifiedMinervaTitle: document.getElementById("classifiedMinervaTitle"),
  classifiedMinervaHint: document.getElementById("classifiedMinervaHint"),
  classifiedSearchToggleBtn: document.getElementById("classifiedSearchToggleBtn"),
  classifiedSearchToggleText: document.getElementById("classifiedSearchToggleText"),
  classifiedSearchWrap: document.getElementById("classifiedSearchWrap"),
  classifiedArchiveCard: document.querySelector(".classified-card-archive"),
  classifiedSearchLabel: document.getElementById("classifiedSearchLabel"),
  classifiedSearchCount: document.getElementById("classifiedSearchCount"),
  classifiedSearchInput: document.getElementById("classifiedSearchInput"),
  classifiedSearchHint: document.getElementById("classifiedSearchHint"),
  classifiedSearchResults: document.getElementById("classifiedSearchResults"),
  classifiedMinervaLists: document.getElementById("classifiedMinervaLists"),
  classifiedInlineDetail: document.getElementById("classifiedInlineDetail"),
  classifiedInlineName: document.getElementById("classifiedInlineName"),
  classifiedInlineWikiLink: document.getElementById("classifiedInlineWikiLink"),
  classifiedInlineCloseBtn: document.getElementById("classifiedInlineCloseBtn"),
  classifiedInlineStatus: document.getElementById("classifiedInlineStatus"),
  classifiedInlineContent: document.getElementById("classifiedInlineContent"),
  classifiedInlineImage: document.getElementById("classifiedInlineImage"),
  classifiedInlineWhereLabel: document.getElementById("classifiedInlineWhereLabel"),
  classifiedInlineWhereList: document.getElementById("classifiedInlineWhereList"),
  classifiedInlineUnlocksLabel: document.getElementById("classifiedInlineUnlocksLabel"),
  classifiedInlineUnlocks: document.getElementById("classifiedInlineUnlocks"),
  visitCounterBadge: document.getElementById("visitCounterBadge"),
  visitCounterIcon: document.querySelector(".visit-counter-icon"),
  visitCounterLabel: document.getElementById("visitCounterLabel"),
  visitCounterValue: document.getElementById("visitCounterValue"),
  visitCounterHint: document.getElementById("visitCounterHint"),
  visitCounterMobileBadge: document.getElementById("visitCounterMobileBadge"),
  visitCounterMobileIcon: document.querySelector(".visit-counter-mobile-icon"),
  visitCounterMobileLabel: document.getElementById("visitCounterMobileLabel"),
  visitCounterMobileValue: document.getElementById("visitCounterMobileValue"),
  visitCounterMobileHint: document.getElementById("visitCounterMobileHint"),
  filesVisitCounterMobileCard: document.getElementById("filesVisitCounterMobileCard"),
  filesVisitCounterMobileIcon: document.getElementById("filesVisitCounterMobileIcon"),
  filesVisitCounterMobileLabel: document.getElementById("filesVisitCounterMobileLabel"),
  filesVisitCounterMobileValue: document.getElementById("filesVisitCounterMobileValue"),
  filesVisitCounterMobileHint: document.getElementById("filesVisitCounterMobileHint"),
  filesAuthCharacterCaption: document.getElementById("filesAuthCharacterCaption"),
  filesAuthConsoleStatus: document.getElementById("filesAuthConsoleStatus"),
  footerText: document.getElementById("footerText")
};
